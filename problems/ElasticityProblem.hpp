...
//   const int numVertices = cellType->getVertexCount();
  ...
  { // Elastic Modulus
    ...
    //Teuchos::ParameterList& paramList = material_db_->getElementBlockSublist(elementBlockName,"Elastic Modulus");
    p->set<Teuchos::ParameterList*>("Parameter List", &paramList);
  ...
  { // Poissons Ratio
    ...
    //Teuchos::ParameterList& paramList = material_db_->getElementBlockSublist(elementBlockName,"Poissons Ratio");
    p->set<Teuchos::ParameterList*>("Parameter List", &paramList);
  ...
  { //Psi1
    Teuchos::RCP<Teuchos::ParameterList> p = Teuchos::rcp(new Teuchos::ParameterList("Psi1 parameters"));
    
    double psi1_initial(0.0);

    p->set<RCP<ParamLib>>("Parameter Library", paramLib);
    Teuchos::ParameterList& param_list = params->sublist("Initial Psi1"); 
    //Teuchos::ParameterList& param_list = material_db_->getElementBlockSublist(elementBlockName,"Initial Psi1");     
    psi1_initial = param_list.get<double>("Psi1");

    // Input
    p->set<std::string>("Psi1 Name","Psi1");
    p->set<Teuchos::ParameterList*>("Parameter List", &param_list); 

    //Output
    p->set<std::string>("Psi1 Name","Psi1");

    ev = Teuchos::rcp(new LCM::Psi1<EvalT,PHAL::AlbanyTraits>(*p,dl));
    fm0.template registerEvaluator<EvalT>(ev);

    p = stateMgr.registerStateVariable("Psi1", dl->qp_scalar,
				       dl->dummy, elementBlockName, "scalar", psi1_initial, true);
  ...
  { //Temperature
    Teuchos::RCP<Teuchos::ParameterList> p = Teuchos::rcp(new Teuchos::ParameterList("Temperature parameters"));
    ...
    ev = Teuchos::rcp(new LCM::Temperature<EvalT,PHAL::AlbanyTraits>(*p,dl));
    fm0.template registerEvaluator<EvalT>(ev);

    p = stateMgr.registerStateVariable("Temperature", dl->qp_scalar,
				       dl->dummy, elementBlockName, "scalar", temperature_initial, true);
  ...
  { //Reference_Temperature
    Teuchos::RCP<Teuchos::ParameterList> p = Teuchos::rcp(new Teuchos::ParameterList("Reference_Temperature parameters"));
    
    double reference_temperature_initial(0.0);
    p->set<RCP<ParamLib>>("Parameter Library", paramLib);

    Teuchos::ParameterList& param_list = params->sublist("Ref Temperature"); 
    //Teuchos::ParameterList& param_list = material_db_->getElementBlockSublist(elementBlockName,"Ref Temperature"); 
    reference_temperature_initial = param_list.get<double>("Value");

    // Input
    p->set<std::string>("Reference_Temperature Name","Reference_Temperature");
    p->set<std::string>("Psi1 Name","Psi1");
    p->set<std::string>("Temperature Name","Temperature");
    p->set<std::string>("QP Coordinate Vector Name", "Coord Vec");
    p->set<Teuchos::ParameterList*>("Parameter List", &param_list); 
    ...
    ev = Teuchos::rcp(new LCM::Reference_Temperature<EvalT,PHAL::AlbanyTraits>(*p,dl));
    fm0.template registerEvaluator<EvalT>(ev);

    p = stateMgr.registerStateVariable("Reference_Temperature", dl->qp_scalar,
				       dl->dummy, elementBlockName, "scalar", reference_temperature_initial, true);
  ...
  { // Strain
    ...
    p->set<RCP<ParamLib>>("Parameter Library", paramLib);
    Teuchos::ParameterList& param_list = params->sublist("Thermal Expansion Coefficient");
    //Teuchos::ParameterList& param_list = material_db_->getElementBlockSublist(elementBlockName, "Thermal Expansion Coefficient");
